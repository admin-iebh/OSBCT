...
SPEC = {
 ...
 '05Vin05': {
   ...
   # NOTE five more corpus paragraphs have the same shape (ord146, 147, 225,
   # 238, 495) and are NOT hidden: those pair, so the counts and the `n`
   # sequence stay consistent with them in place, and hiding them would break
   # the pairing rather than fix it.  Recorded rather than tidied.
   'backmatter': [227],
...
HOMAGE = re.compile(r'Namo [Tt]assa Bhagavato Arahato Sammāsambuddhassa')
...
def nid_build(pages, paras, title, p0, p1, o0, o1,
    ...
    def add_prose(t, new_para):
        ...
        after[-1] = (after[-1][:-1] + t if after[-1].endswith('-')
                     else after[-1] + ' ' + t)
...
def kat_is_colo(t, ind=None, body=0):
    ...
    if SPEC[VOL].get('heads_by_form'):
        ...
        return (t.endswith('.') and ',' not in t and len(t.split()) <= 6
                and ind is not None
                and (ind >= body + 8
                     or (bool(NIDCOLO.search(t)) and bool(NIDSECT.search(t)))))
...
def kat_is_head(core, printed=True):
    ...
    if SPEC[VOL].get('heads_by_form') and printed:
        return len(core.split()) <= 6 and not core.endswith('.')
    ...
    for pg in range(p0, p1 + 1):
        ...
        for _li, (ind, t) in enumerate(lines):
            ...
            # 29Abhi01, 30Abhi02 and 36-40, all byte-identical.
                  ...
                  and not (_runon
                           and (_li in _runidx
                                or (_li and lines[_li - 1][1].rstrip().endswith(',')))
            ...
            # the 2 every shipped volume was measured at.
                  ...
                  and len(core.split()) <= SPEC[VOL].get('head_words', 2)
                  and '.' not in core[:-1]
                  and (ind >= body + 14
        ...
        while i < len(kinds):
            ...
            if run and SPEC[VOL].get('display_prose'):
                ...
                tail = j
                if not all(re.search(r'[,.]$', lines[k][1])
...
def kat_build(pages, paras, title, p0, p1, o0, o1,
    ...
    rec = {'book': title, 'mode': 'katha',
           'printed_units': len(printed), 'corpus_paras': len(ords),
    ...
    def add_prose(t, new_para, num=None):
        ...
        if new_para or not open_prose or prev is None:
            ...
            open_prose = True
            return
        joined = prev[:-1] + t if prev.endswith('-') else prev + ' ' + t
    ...
    for it in items:
        ...
        if cur is None or pend_heads:
            ...
            if (kind != 'popen' or (_pv is not None and _pv.endswith('-'))) \
                    and _pv is not None:
                _j = _pv[:-1] + it[1] if _pv.endswith('-') else _pv + ' ' + it[1]
...
def build():
    ...
    # tidy: an uddāna block with a label but no lines is a stray label.  A
    # block carrying a HEAD and no lines is not — that is a printed heading
    # immediately followed by another heading, which is how 28Khu11 sets
    # "Meṇḍakapañhārambhakathā / Aṭṭhamantaparivajjanīyaṭṭhāna" and
    # "Ācariyaguṇa / Upāsakaguṇa".  This filter was silently deleting the
    # first of each pair.
    for k in list(uddana):
        ...
        uddana[k] = [b for b in uddana[k] if b.get('lines') or b.get('head')]
        if not uddana[k]:
            del uddana[k]
