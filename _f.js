...
[...w.document.querySelectorAll('.row')].find(r=>r.textContent.trim()==='1. Uragavagga')?.click();
...
// Āḷavaka ord885: number 183 should be on the verse (inside .gatha), not before intro prose
const p885=w.document.querySelector('#p-18Khu01-885');
if(p885){
  const g=p885.querySelector('.gatha');
  console.log('Āḷavaka 183 inside verse .gatha:', g? /^183\./.test(g.textContent.trim()):'no gatha');
  const firstProse=p885.querySelector('.gatha-after');
  console.log('  intro prose (Evaṁ me sutaṁ) has NO leading number:', firstProse? !/^\s*183/.test(firstProse.textContent):'?');
}
})();
