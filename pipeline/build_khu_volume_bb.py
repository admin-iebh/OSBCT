...
# ---------------------------------------------------------------------------
# BLOCKBREAK: the printed line break, taken from the page's own leading.
#
# `add_prose` keeps a printed line as its own drawn line only when its kind is
# 'popen', and `kat_items` decides that from INDENT ALONE -- so the second line
# of a Dhammasangani dyad reads as a continuation and is joined away.  Measured:
# 672 display blocks, 865 printed lines (_xc/hy1/FINDINGS.md 16).
#
# The block map (_xc/hy1/bjoin2/, joined to pline at 99.93% of body lines) says
# whether a printed line BEGINS A NEW LINE INSIDE A BLOCK THE PAGE SETS APART.
# Where it does, the break is structural and must survive; inside a prose
# paragraph it is a wrap and joining is right.
#
# OFF unless BLOCKBREAK=1, so this file is its own negative control.
# ---------------------------------------------------------------------------
...
def _bb_key(s):
    return _BB_NRM.sub('', s or '')


class _BBCursor:
    """Walks the block map in printed order, in step with `items`.

    !!! KEYED BY THE RAW pdftotext PAGE, which is what this builder's item
    stream carries: `pdf_pages()` splits `pdftotext -layout` on \f and
    `page_lines(pages, i)` indexes `pages[i-1]`.  The first version of this
    cursor keyed on `pline`'s ACCEPTED-page index instead -- pline numbers only
    the pages `split_page` keeps -- and 88% of lines went unmatched with ZERO
    display lines found, while the negative control still passed because the
    flag was simply never firing.  `_xc/hy1/blocks2/` is already keyed by the
    raw page, so the join is not needed here at all.

    Position, never text alone: the Abhidhamma repeats a line verbatim dozens of
    times on a page.
    """
    ...
    def at(self, pg, txt):
        """(is_block_start, kind) for this printed line, or None if unaligned."""
        k = _bb_key(txt)
        rows = self.pages.get(pg)
